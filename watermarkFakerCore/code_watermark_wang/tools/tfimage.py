...
# 我自己添加的函数========
decode_bmp = create_op(
    func=tf.image.decode_bmp,
    contents=tf.placeholder(tf.string),
)
# =====================
...
def load(path):
    ...
    _, ext = os.path.splitext(path.lower())  # os.path.splitext的作用为将文件名与拓展名分开
    ...
    if ext == ".jpg":
        ...
        image = decode_bmp(contents=contents)
    else:
